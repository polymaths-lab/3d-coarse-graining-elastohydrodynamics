...
%of velocities AND angular velocities
% Q [w1, ..., wN] = [v1, w1, ..., vNn, wNn]
...
for fil = 1:Nfil
    ...
    for i = 2:N
        ...
        for k = 1:n
            ...
            Q(3*(sn-1)+1:3*sn, 3+3+3*(fil-1)*N*n+1:3+3+3*fil*N) = Q(3*(sn-1-1)+1:3*(sn-1), 3+3+3*(fil-1)*N*n+1:3+3+3*fil*N);
